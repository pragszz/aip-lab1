...
class Ticket5(BaseModel):
    ...
    "Urgency of a ticket on a scale of 1 to 5:"
    "1 -> Very Low Priority, handle whenever"
    "2 -> Not high priority, handle in a few weeks"
    "3 -> Medium priority, handle in a few days"
    "4 -> High Priority, needs to be addressed within 24 hours"
    "5 -> Very High and Critical Priority, needs to addressed immediately"
    ))
...
def failing_fields(payload: dict) -> list[str]:
    ...
    try: 
        ...
        return sorted({err["loc"][0] for err in e.errors()})
...
class TicketRecord(BaseModel):
    ...
    evidence: str = Field(max_length = 200, description="This is the part of the ticket which determines the urgency category")
    category: CATEGORIES = Field(
        description=(
            "The category of the support ticket should be one of: "
            "'billing' (payments, invoices, refunds), "
            "'claims' (filing or status of an insurance claim), "
            "'policy_change' (adding/removing coverage, updating policy details), "
            "'technical' (issues with the app, website, or login), "
            "'complaint' (dissatisfaction with service, staff, or outcomes), "
            "'information' (general questions not tied to a specific request)."
        )
    )
    urgency: int = Field(ge =1, le = 5, description= (
        "Urgency of a ticket on a scale of 1 to 5:"
    ...
    policy_number: str | None = Field(default=None, pattern=r"^AUR-\d{7}$", description=(
        "The exact format of the policy number needs to be AUR-1234567, where after AUR there can be any 7 digits." \
        "There should be no whitespaces and this is the only way to determine the policy number. It is also unique."
    ))
